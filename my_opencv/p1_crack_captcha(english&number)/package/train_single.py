...
y_ = tf.placeholder(tf.float32, [None, 3096])  # 36*36+36   (所有可能结果的组合)
...
w_fc2 = weight_variable([1024, 3096])
b_fc2 = bias_variable([3096])
...
for i in range(3000):
    ...
    if i % 100 == 0:
        train_accuracy = accuracy.eval(feed_dict={x: img, y_: y_,
                                                  keep_prob: 1.0})
        print(f"进行{i}次训练, 准确率是{train_accuracy}%")
    saver.save(sess, './my_moduls/my_model')
